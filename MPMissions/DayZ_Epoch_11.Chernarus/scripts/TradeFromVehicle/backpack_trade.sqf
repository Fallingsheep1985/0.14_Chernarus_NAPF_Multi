...
private ["_backpack","_started","_canceltrade","_finished","_animState","_isMedic","_isOk",
		"_goStart","_tPassed","_sTime","_display","_pStartPos","_sWepCounts",
...
_sWepCounts = _backpack call tfv_fnc_wCount;
_canceltrade = false;
_preTestCargoCounts = getWeaponCargo _backpack;
if (count(_preTestCargoCounts select 0) == 0) exitWith { tfv_IS_TRADING = false; systemChat format [tfv_NO_BP_WEAPONS,typeOf _backpack];};
...
while {!_goStart} do {
	...
	if (([_vStartPos,_sWepCounts,_backpack,_pStartPos] call tfv_fnc_checkTrade)) exitWith {    //Check to be sure that the player is not swindling us!
	    _canceltrade = true;
		titleText [tfv_CANCELLED_TRADE,"PLAIN DOWN"];
	};
};
if (_canceltrade) exitWith { tfv_IS_TRADING = false; };

sleep 0.5;
_trader = _this select 3 select 0;
_traderWeapons = _trader call tfv_fnc_findTrWeapons;
_weaponsActual = _backpack call tfv_fnc_aConcat;
...
_nul = [_bars,_rejects,_backpack] call tfv_fnc_payTrade;
{ systemChat format [tfv_SIDENOTE,_x]; } forEach _bars;

diag_log(format ["%1 %2" ,name player, _display]);
_display = format [tfv_SALE_SUCCESS_BP_STRING,(count _weaponsActual) - (count _rejects)];
titleText [_display,"PLAIN DOWN"];
tfv_IS_TRADING = false;