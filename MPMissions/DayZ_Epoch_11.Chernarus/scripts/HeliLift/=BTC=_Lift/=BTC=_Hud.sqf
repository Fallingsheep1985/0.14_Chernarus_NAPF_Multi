...
if (BTC_Hud_Cond) then {
	BTC_liftHudId = _chopper addAction [("<t color=""#ED2744"">" + ("Lift Hud Off") + "</t>"),"scripts\HeliLift\=BTC=_Lift\=BTC=_Hud.sqf", "", 0, false, false];
} else {
	BTC_liftHudId = _chopper addAction [("<t color=""#ED2744"">" + ("Lift Hud On") + "</t>"),"scripts\HeliLift\=BTC=_Lift\=BTC=_Hud.sqf", "", 0, false, false];
};