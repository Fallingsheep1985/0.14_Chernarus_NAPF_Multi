...
_waitScript =  [] spawn {
	...
	_bodyCheck = {
		...
		_bodies = [];
		{if ((_x distance [10153.301,-4593.061,0]) < 15) then {deleteVehicle _x};if (!isNull _x) then {if ((_x getVariable["bodyName",name _x]) == (name player)) then {_bodyPos = (getPosATL _x);_bodies set [count _bodies,_bodyPos];};};} count allDead;
...
if (spawnSelect == 40) exitWith {
	...
	if (haloSelect == 1) then {player setPosATL [_position select 0,_position select 1,1000];[player, 1000] execVM "fixes\fn_HALO.sqf";} else {player setPosATL _position; player setvelocity [0,120*0.8,0];
player setdir 0;};
};
